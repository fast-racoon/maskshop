...
<div class="lineh"></div>	
		<h2 class="bxr-font-color"><span><?if(!empty($arParams["BXREADY_LIST_PAGE_BLOCK_TITLE_GLYPHICON"])) echo "<i class='fa ".$arParams["BXREADY_LIST_PAGE_BLOCK_TITLE_GLYPHICON"]."'></i>";?><?=$arParams["BXREADY_LIST_PAGE_BLOCK_TITLE"]?></span></h2>
	<?endif;?>
	...
	<div id="bxr-mobile-name-<?=$arItem["ID"]?>"
				 class="bxr-mobile-names bxr-color-button"
				   ...
				   data-unicum="<?=$unicum?>">
					<?=$arItem["NAME"]?> <span class="hidden-xs">(<?=intval(count($arItem["PROPERTY_ITEMS_VALUE"]))?>)</span>
			</div>
	</div>
				...
				load: function(idC, type, mobileMode){
					...
					if (BXReady.Market.bestsellersAjaxUrl.length > 0 && BXReady.Market.Bestsellers.iblockID > 0){
						...
						if ($('#rec_tab'+idC).data('state') != 'load'){
							...
							$('#c'+idC).css('display', 'block');
							<?if ($arParams["BXREADY_LIST_SLIDER"] == "Y"):?>
							$('.bestseller-slick-animation').slick('destroy');
							initBestsellerSlider($('#c'+idC).data('slider'));
							<?endif;?>
						}
