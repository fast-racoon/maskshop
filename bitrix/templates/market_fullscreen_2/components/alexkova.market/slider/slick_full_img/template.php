...
<?if (count($arResult["ITEMS"])<=0) return;?>
    <div class="bxr-slider hidden-xs <? if(isset($arParams['SLIDER_FULL_SCREEN']) && $arParams['SLIDER_FULL_SCREEN']=="N") {echo "container";} ?>" accesskey="" style="position: relative">
    <?foreach ($arResult["ITEMS"] as $key => $item):?> 
		...
		<div class="container <?=$location;?>" style="background: url('<?=$item["DETAIL_PICTURE"]["SRC"]?>') no-repeat; background-size: 100%;"
	...
	<div class="bxr-slider-mob hidden-sm hidden-md hidden-lg <? if(isset($arParams['SLIDER_FULL_SCREEN']) && $arParams['SLIDER_FULL_SCREEN']=="N") {echo "container";} ?>" accesskey="" style="position: relative">
    <?foreach ($arResult["ITEMS"] as $key => $item):?> 
        <?
            $target = "";
            ($item["PROPERTIES"]["NEW_TAB"]["VALUE"] == "Y") ? $target = "target='_blank'" : $target = "target='_self'";
        ...
        ?>
		<div class="container <?=$location;?>" style="background: url('<?if($item["PROPERTIES"]["IMG_MOB"]["VALUE"] != ""){echo CFile::GetPath($item["PROPERTIES"]["IMG_MOB"]["VALUE"]);}else{echo $item["DETAIL_PICTURE"]["SRC"];}?>') no-repeat; background-size:cover;"
            alt="<?=$item["NAME"]?>" title="<?=$item["NAME"]?>">
			<div class="row">
            <?
                $link = "/";
    ...
    window.BXReady.Market.SlickSliderTop = {
        init: function(){
            ...
            $('.bxr-slider-mob').slick({
                ...
                autoplay: true,
				nextArrow: false,/*'<button type="button" class="slick-next bxr-bg-hover-dark-flat"><i class="fa fa-angle-right"></i></button>',*/
				prevArrow: false,/*'<button type="button" class="slick-prev bxr-bg-hover-dark-flat"><i class="fa fa-angle-left"></i></button>',*/
            });

			$('.bxr-slider-mob').css("visibility", "visible");
        }
    }
