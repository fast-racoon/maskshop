...
/*
COption::SetOptionString("alexkova.market","bxr_org_name",$arResult["NAME"], false, "s1");
COption::SetOptionString("alexkova.market","bxr_org_description",$arResult["PREVIEW_TEXT"], false, "s1");
COption::SetOptionString("alexkova.market","bxr_org_logo",$arResult["DETAIL_PICTURE"]["SRC"], false, "s1");
*/
...
<script>
    $(document).ready(function() {
        ...
        paramsSKU = [];
        <?foreach ($arResult["OFFERS"][$arResult["FIRST_SKU_SELECT"]]["PROPERTIES"] as $k => $params):?>
            <?if(!isset($arResult["SKU_PROPS_LIST"][$params["CODE"]])) continue;?>
            paramsSKU["<?=$k?>"] = [];
            paramsSKU["<?=$k?>"].id = "<?=$params["ID"]?>";
            paramsSKU["<?=$k?>"].val = "<?=$params["VALUE"]?>";
            paramsSKU["<?=$k?>"].url = "<?=$arResult["OFFERS"][$arResult["FIRST_SKU_SELECT"]]["DETAIL_PICTURE"]["SRC"]?>";
        <?endforeach;?>
        selectSKU("<?=$arResult["OFFERS_VIEW"];?>", "<?=$arResult["FIRST_SKU_SELECT"]?>", paramsSKU); 
    });
</script>
...
<? if (is_array($arResult["PROPERTIES"][$arParams["LINK_PROPERTY_SID"]]["VALUE"]) && count($arResult["PROPERTIES"][$arParams["LINK_PROPERTY_SID"]]["VALUE"]) > 0) { ?>
    ...
    <div class="row" id="bxr-accessories-block-wrap">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <h3><?=$arResult["PROPERTIES"][$arParams["LINK_PROPERTY_SID"]]['NAME']?></h3>
            <?
