                            ...
                            <?if (in_array($priceIndex, $arResult["PRICES_ALLOW"])):?>
                                <div class="bxr-market-item-price bxr-format-price 
                                    ...
                                    <?if ($boolDiscountShow && $price[$itemRangeKey]['PRICE'] != $price[$itemRangeKey]['DISCOUNT_PRICE']) {?>
                                        <span class="bxr-market-old-price hidden-sm hidden-xs" id="<? echo $arItemIDs['OLD_PRICE']; ?>"><?=CurrencyFormat($price[$itemRangeKey]['PRICE'],$price[$itemRangeKey]['CURRENCY'])?></span>
                                        <span class="bxr-market-old-price hidden-lg hidden-md" style="font-size: 22px; float: left;" id="<? echo $arItemIDs['OLD_PRICE']; ?>_mob"><?=CurrencyFormat($price[$itemRangeKey]['PRICE'],$price[$itemRangeKey]['CURRENCY'])?></span>
									<?}else{?>
										<script>
											//делаем по центру цену, если нет скидок
											$(document).ready(function() {
												if($(window).width() < 600)
												{
													$('.bxr-market-current-price').attr('style', 'float: none;');
												}
											});
										</script>
									<?}?>
                                    <!--current price with all discounts-->
                                    <span data-id="pr" class="bxr-market-current-price bxr-market-format-price" id="<? echo $arItemIDs['PRICE']; ?>"><?=CurrencyFormat($price[$itemRangeKey]['DISCOUNT_PRICE'],$price[$itemRangeKey]['CURRENCY']) ?><?=($showMeasure?GetMessage("CATALOG_MEASURE_DELIMITER").$arResult["CATALOG_MEASURE_RATIO"].$arResult["CATALOG_MEASURE_NAME"]:'')?></span>
                        ...
                        <?  foreach ($price as $priceRangeIndex => $priceRange) {?>
                                <div class="bxr-market-item-price bxr-format-price <?if (count($arResult["PRICE_MATRIX"]["ROWS"]) == 1) echo "bxr-no-range-price"?>
									...
									<?/*<span class="bxr-market-old-price" id="<? echo $arItemIDs['OLD_PRICE']; ?>"><?=CurrencyFormat($priceRange['PRICE'],$priceRange['CURRENCY'])?></span>*/?>
										<span class="bxr-market-old-price hidden-sm hidden-xs" id="<? echo $arItemIDs['OLD_PRICE']; ?>"><?=CurrencyFormat($priceRange['PRICE'],$priceRange['CURRENCY'])?></span>
                                        <span class="bxr-market-old-price hidden-lg hidden-md" style="font-size: 22px; float: left;" id="<? echo $arItemIDs['OLD_PRICE']; ?>_mob"><?=CurrencyFormat($priceRange['PRICE'],$priceRange['CURRENCY'])?></span>
									...
									<?}?>
                                    <!--current price with all discounts-->
                                    <span data-id="pr"  class="bxr-market-current-price bxr-market-format-price" id="<? echo $arItemIDs['PRICE']; ?>"><?=CurrencyFormat($priceRange['DISCOUNT_PRICE'],$priceRange['CURRENCY'])?><?=($showMeasure?GetMessage("CATALOG_MEASURE_DELIMITER").$arResult["CATALOG_MEASURE_RATIO"].$arResult["CATALOG_MEASURE_NAME"]:'')?></span>
                                    <div class="clearfix"></div>
                                </div>
            ...
            <?if (in_array($arResult["CAT_PRICES"][$priceCode]["ID"], $arResult["PRICES_ALLOW"])):?>
                <div class="bxr-market-item-price bxr-format-price <?if (!$priceNameShow || count($arResult["PRICES"]) == 1) echo 'bxr-market-price-without-name'?>">
                    ...
                    <?if ($boolDiscountShow && $arPrice['VALUE'] != $arPrice['DISCOUNT_VALUE']) {?>
						...
						<span class="bxr-market-old-price hidden-sm hidden-xs" id="<? echo $arItemIDs['OLD_PRICE']; ?>"><?=$arPrice['PRINT_VALUE']?></span>
						<span class="bxr-market-old-price hidden-lg hidden-md" style="font-size: 22px; float: left;" id="<? echo $arItemIDs['OLD_PRICE']; ?>_mob"><?=$arPrice['PRINT_VALUE']?></span>
					<?}else{?>
						<script>
							//делаем по центру цену, если нет скидок
							$(document).ready(function() {
					...
					<?}?>
                    <!--current price with all discounts-->
                    <span data-id="<?=($arPrice['DISCOUNT_VALUE'])?number_format($arPrice['DISCOUNT_VALUE'], 0, '', ''):0?>" class="bxr-market-current-price bxr-market-format-price" id="<? echo $arItemIDs['PRICE']; ?>"><?=CurrencyFormat($arPrice['DISCOUNT_VALUE'],$arPrice["CURRENCY"]) ?><?=($showMeasure?GetMessage("CATALOG_MEASURE_DELIMITER").$arResult["CATALOG_MEASURE_RATIO"].$arResult["CATALOG_MEASURE_NAME"]:'')?></span>
                    <div class="clearfix"></div>
                </div>
		<meta itemprop="price" content="<?=($arPrice['DISCOUNT_VALUE'])?number_format($arPrice['DISCOUNT_VALUE'], 0, '', ''):0?>">
        ...
        <?foreach($arResult["MIN_PRICE"] as $priceCode => $arPrice):?>
            <?if (in_array($arResult["CAT_PRICES"][$priceCode]["ID"], $arResult["PRICES_ALLOW"])):?>
                <div class="bxr-market-item-price bxr-format-price <?if (!$priceNameShow || count($arResult["MIN_PRICE"]) == 1) echo 'bxr-market-price-without-name'?>">
					...
					<?}?>
                    <!--current price with all discounts-->
                    <span data-id="<?=($arPrice['DISCOUNT_VALUE'])?$arPrice['DISCOUNT_VALUE']:0?>"  class="bxr-market-current-price bxr-market-format-price" id="<? echo $arItemIDs['PRICE']; ?>"><?=CurrencyFormat($arPrice['DISCOUNT_VALUE'],$arPrice["CURRENCY"]) ?><?=($showMeasure?GetMessage("CATALOG_MEASURE_DELIMITER").$arResult["CATALOG_MEASURE_RATIO"].$arResult["CATALOG_MEASURE_NAME"]:'')?></span>
                    <?if ($showFrom) {?>
