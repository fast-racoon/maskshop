...
<?//echo '<pre>';echo print_r($arResult["DISPLAY_PROPERTIES"]); echo '</pre>';?>
	...
	<?foreach($arParams["PREVIEW_DETAIL_PROPERTY_CODE"] as $propertyCode):
		$arProperty = $arResult["DISPLAY_PROPERTIES"][$propertyCode];
                if (!is_array($arProperty["DISPLAY_VALUE"]) && $arProperty["DISPLAY_VALUE"]) {?>
                            <?if ($arProperty['CODE'] == 'MANUFACTURER'){
	                            $str = $arProperty["VALUE"];
								$new_link = str_replace(" ","",$str);
                            ?>
		                        <tr itemprop="additionalProperty" itemscope itemtype="http://schema.org/PropertyValue">
		                                <td class="bxr-props-name"><span itemprop="name"><?=$arProperty["NAME"]?></span></td>
		                                <td class="bxr-props-data"><span itemprop="value"><a href="/brands/<?=$new_link?>/"><?=$arProperty["DISPLAY_VALUE"]?></a></span></td>
		                        </tr>                          
                            <?}else{?>
                                                    <tr itemprop="additionalProperty" itemscope itemtype="http://schema.org/PropertyValue">
                                <td class="bxr-props-name"><span itemprop="name"><?=$arProperty["NAME"]?></span></td>
...
<?//добавляю скрытое поле "страна производитель"?>
<tr itemprop="additionalProperty" itemscope="" itemtype="http://schema.org/PropertyValue" style="display:none">
	<td class="bxr-props-name"><span itemprop="name">Cтрана производитель</span></td>
