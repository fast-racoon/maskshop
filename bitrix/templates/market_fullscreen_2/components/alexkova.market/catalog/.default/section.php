...
/*
$rsSections = CIBlockElement::GetElementGroups($arResult['ID'], true); //определяем раздел товара	

while($arSection = $rsSections->Fetch()) $arSections[] = $arSection["CODE"];	//собираем массив разделов с таким символьным кодом	
//if (($USER->IsAuthorized())&&in_array(1,CUser::GetUserGroup(CUser::GetID()))||in_array(5,CUser::GetUserGroup(CUser::GetID()))){ 
//echo var_dump($arSections["CODE"]);
//}
if(!in_array($arParams['SECTION_CODE'], $arSections)){ //перебираем массив, если такого раздела нет, выводим 404		   
	@define("ERROR_404", "Y");	//добавляем константу		   
	if($arParams["SET_STATUS_404"]==="Y") CHTTP::SetStatus("404 Not Found"); //открываем 404		
}
*/
