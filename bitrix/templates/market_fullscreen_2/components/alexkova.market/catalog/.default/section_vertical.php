...
// Получаем СЕО страницы для фильтра
$SEO_IBLOCK_ID = getIbIdByCode('seo_filter');
$arFilter = ["IBLOCK_ID" => $SEO_IBLOCK_ID, "ACTIVE"=>"Y", "PROPERTY_LINK"=>$currentPageUrl];
$res = CIBlockElement::GetList(Array(), $arFilter, false, false, ["ID", "IBLOCK_ID", "NAME", "PREVIEW_TEXT"]);
if($arSEO = $res->Fetch()) {
	$arMeta = (new Bitrix\Iblock\InheritedProperty\ElementValues($arSEO['IBLOCK_ID'], $arSEO['ID']))->getValues();
}
?>
...
<div class="<?=(($isSidebar) ? "col-md-9 col-sm-8" : "col-xs-12")?>">
    <h1 style="border-bottom: 1px solid #e5e5e5;"><?$APPLICATION->ShowTitle(false);?></h1>
    <div class="row">
        ...
        <div class="col-xs-12">
			...
			if(empty($arSEO)) {
                global $arSEOFilter;
                $arSEOFilter['PROPERTY_SECTION'] = $arCurSection['ID'];
                $APPLICATION->IncludeComponent(
                    "bitrix:news.list",
                    "row",
                    array(
			...
			if ($arCurSection["DESCRIPTION"] && $arParams["SHOW_SECTION_DESC"] == "top" && strpos($APPLICATION->GetCurPageParam(), 'PAGEN_') === false) {?>
                <div class="bxr-section-desc">
					<?=($arSEO['PREVIEW_TEXT'] ? $arSEO['PREVIEW_TEXT'] : $arCurSection["DESCRIPTION"])?>
                </div>
			<?}

			$APPLICATION->IncludeComponent(
				"alexkova.market:sort.panel",
				"lastCatalog_masckshop",
				array(
					"IBLOCK_ID" => "11",
					"COMPONENT_TEMPLATE" => "lastCatalog_masckshop",
					"THEME" => "solid",
			...
			// Устанавливаем СЕО заголовки
			if(!empty($arMeta)) {
				$APPLICATION->SetTitle(($arMeta["ELEMENT_PAGE_TITLE"] ? $arMeta["ELEMENT_PAGE_TITLE"] : $arSEO["NAME"]));
				$APPLICATION->SetPageProperty('title', $arMeta["ELEMENT_META_TITLE"]);
				$APPLICATION->SetPageProperty('keywords', $arMeta["ELEMENT_META_KEYWORDS"]);
				$APPLICATION->SetPageProperty('description', $arMeta["ELEMENT_META_DESCRIPTION"]);
			}
			//echo '<pre>'; echo print_r($arResult["URL_TEMPLATES"]["section"]); echo '</pre>';
			?>
        </div>
		<?
		...
		if ($arCurSection["DESCRIPTION"] && $arParams["SHOW_SECTION_DESC"] == "bottom" && strpos($APPLICATION->GetCurPageParam(), 'PAGEN_') === false) {?>
            <div class="bxr-section-desc">
				<?=($arSEO['PREVIEW_TEXT'] ? $arSEO['PREVIEW_TEXT'] : $arCurSection["DESCRIPTION"])?>
            </div>
		<?}?>
