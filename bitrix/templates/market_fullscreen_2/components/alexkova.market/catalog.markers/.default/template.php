...
<?//echo print_r($arResult)?>
			...
			<?foreach($arResult["MARKERS_LIST"] as $cell):
				...
				<?			
					...
					$arFilter = array("ACTIVE"=>"Y", "IBLOCK_ID"=>11, '!PROPERTY_NEWPRODUCT' =>false);
				...
				?>
					<div class="bxr-markers-group col-md-4" id="marc_tabNEWPRODUCT" data-slide="NEWPRODUCT" data-type="markers"><?=GetMessage('NEWPRODUCT_BUTTON')?></div>
				...
				<?			
					$arSelect = array("ID", "IBLOCK_ID", "NAME");
					$arFilter = array("ACTIVE"=>"Y", "IBLOCK_ID"=>11, '!PROPERTY_SPECIALOFFER' =>false);
					$res = CIblockElement::GetList(array(), $arFilter, false, Array("nPageSize"=>50), $arSelect);
					$i=0;
					while($ob = $res->GetNextElement())
					{$arFields = $ob->GetFields();
				...
				?>				
					<div class="bxr-markers-group col-md-4" id="marc_tabSPECIALOFFER" data-slide="SPECIALOFFER" data-type="markers"><?=GetMessage('SPECIALOFFER_BUTTON')?></div>
					<?
				...
				<?			
					$arSelect = array("ID", "IBLOCK_ID", "NAME");
					$arFilter = array("ACTIVE"=>"Y", "IBLOCK_ID"=>11, '!PROPERTY_RECOMMENDED' =>false);
					$res = CIblockElement::GetList(array(), $arFilter, false, Array("nPageSize"=>50), $arSelect);
					$i=0;
					while($ob = $res->GetNextElement())
					{$arFields = $ob->GetFields();
				...
				?>					
					<div class="bxr-markers-group col-md-4" id="marc_tabRECOMMENDED" data-slide="RECOMMENDED" data-type="markers"><?=GetMessage('RECOMMENDED_BUTTON')?></div>
					<?
				...
				<?			
					$arSelect = array("ID", "IBLOCK_ID", "NAME");
					$arFilter = array("ACTIVE"=>"Y", "IBLOCK_ID"=>11, '!PROPERTY_SALELEADER' =>false);
					$res = CIblockElement::GetList(array(), $arFilter, false, Array("nPageSize"=>50), $arSelect);
					$i=0;
					while($ob = $res->GetNextElement())
					{$arFields = $ob->GetFields();
				...
				?>					
					<div class="bxr-markers-group col-md-4" id="marc_tabSALELEADER" data-slide="SALELEADER" data-type="markers"><?=GetMessage('SALELEADER_BUTTON')?></div>
					<?
		...
		case "ACTION":?>
			<div class="col-xs-4 hidden-lg hidden-md hidden-sm">
				<div id="bxr-mobile-name-SPECIALOFFER"
					 ...
					 data-unicum="<?=$unicum?>" style="padding-top: 10px;">
		...
		if (typeof(BXReady.Market) == 'object'){
			...
			function initMarkerSlider(unicumID){
				...
				$('#sl_'+unicumID).slick({
					...
					vertical: true,
					<?}?>
					slidesToShow: 5<?//=$colToElem[$arParams["BXREADY_LIST_LG_CNT"]]?>,
					slidesToScroll: 1,
					prevArrow: prevBtn,
					nextArrow: nextBtn,
					responsive: [
