...
PR($arResult, 1,1);
//print_r($arResult);
ShowMessage($arResult['ERROR_MESSEGE']);
	...
	<table class="data-table bx-changepass-table">
		...
		<tbody>
			<tr>
				...
				<td><input type="text" name="USER_LOGIN" maxlength="50" value="<?=urldecode($_REQUEST["USER_LOGIN"])?>" class="bx-auth-input" /></td>
			</tr>
...
<?endif?>
		...
		<tfoot>
			<tr>
				<td></td>
				<td><input type="submit" name="change_pwd" class="changePswdBtn" value="<?=GetMessage("AUTH_CHANGE")?>" /></td>
			</tr>
		</tfoot>
	</table>
...
window.onload=function(){
	$('font.notetext').html('Пароль успешно изменен.');
}
</script>
</div>