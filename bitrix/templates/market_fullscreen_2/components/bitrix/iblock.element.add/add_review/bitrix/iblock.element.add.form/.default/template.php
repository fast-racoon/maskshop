	...
	<table class="data-table" style="width: 90%">
		...
		<tbody>
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
				<tr>
					<?if(($propertyID != "NAME") && ($propertyID != "161")){?>
					...
					<td>
						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "N":
								...
								?>
						<input type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>" <?if($propertyID == "NAME"){?>value="Îòçûâ" style="display:none;"<?}else if($propertyID == "161"){?>value="<?echo date($DB->DateFormatToPHP(CSite::GetDateFormat("SHORT")), time());?>" style="display:none;"<?}else{?>value="<?=$value?>"<?}?>/><?if(($propertyID != "NAME") && ($propertyID != "161")){?><br /><?}?><?
								...
								?><?if(($propertyID != "NAME") && ($propertyID != "161")){?><br /><?}?><?
								}
							break;
		...
		<tfoot>
			<tr>
				<td colspan="2">
					<input type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" class="bxr-color-button" style="background: #9cca68!important;" />
					<?if (strlen($arParams["LIST_URL"]) > 0):?>
						...
						<input
							...
							class="bxr-color-button"
							style="border: 0px none; outline: medium none; border-radius: 4px; box-shadow: 0px 1px 1px 0px rgba(0,0,0,.3); font-size: 16px; padding: 6px 25px 10px; margin: 10px; text-decoration: none;"
						>
					<?endif?>
				</td>
			</tr>
