...
<div class="col-lg-7 col-xs-12 section no_padding">
...
<select class="block_select_paysystem" id="block_select_paysystem" onchange="BX('ID_PAY_SYSTEM_ID_'+$('#block_select_paysystem option:selected').attr('data-id')).checked=true;changePaySystem(); $('.pay_description').hide(); $('#pay_description_'+$('#block_select_paysystem option:selected').attr('data-id')).show();" >
	<?foreach($arResult["PAY_SYSTEM"] as $arPaySystem)
	{
		if (strlen(trim(str_replace("<br />", "", $arPaySystem["DESCRIPTION"]))) > 0 || intval($arPaySystem["PRICE"]) > 0)
		{?>
			<option
				data-id="<?=$arPaySystem["ID"];?>"
				<?if ($arPaySystem["CHECKED"]=="Y" && !($arParams["ONLY_FULL_PAY_FROM_ACCOUNT"] == "Y" && $arResult["USER_VALS"]["PAY_CURRENT_ACCOUNT"]=="Y")) echo " selected";?>
			>
...
<br>
	...
	<?foreach($arResult["PAY_SYSTEM"] as $arPaySystem)
	{
		if (strlen(trim(str_replace("<br />", "", $arPaySystem["DESCRIPTION"]))) > 0 || intval($arPaySystem["PRICE"]) > 0)
		{?>
			<div id="pay_description_<?=$arPaySystem["ID"];?>" class="pay_description" <?if ($arPaySystem["CHECKED"]!="Y"){?>style="display:none"<?}?>><?echo $arPaySystem["DESCRIPTION"];?></div>
		<?}
	}?>
							...
							<!--	<div class="bx_logotype">
								...
								<div class="bx_description">
									<?if ($arParams["SHOW_PAYMENT_SERVICES_NAMES"] != "N"):?>
										<strong style="float: none;"><?=$arPaySystem["PSA_NAME"];?></strong>
									<?endif;?>
									<p>
										<?
...
<div class="col-lg-5 hidden-xs hidden-sm">
	<div class="bx_section">
		<h4><img class="r_ico" src="<?=$this->GetFolder()?>/images/r_ico_2.png">&nbsp;&nbsp;&nbsp;БЕЗОПАСНОСТЬ</h4>
		<p>Безопасность платежей гарантируется использованием SSL-протокола. Данные вашей банковской карты надежно 
