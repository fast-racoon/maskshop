	...
	if (is_array($arResult["ORDER_PROP"]["USER_PROFILES"]) && !empty($arResult["ORDER_PROP"]["USER_PROFILES"])):
		if ($arParams["ALLOW_NEW_PROFILE"] == "Y"):
            ...
            <div class="bx_block r3x1">
                <select name="PROFILE_ID" style="display: none;" id="ID_PROFILE_ID" onChange="SetContact(this.value)">
...
<div class="col-lg-5 hidden-xs hidden-sm" style="min-height: 393px;">
    <div class="bx_section">
		<?if(!$USER->IsAuthorized()):?>
            <h4><img class="r_ico" src="<?=$this->GetFolder()?>/images/r_ico_1.png">&nbsp;&nbsp;&nbsp;ВОЙТИ В ЛИЧНЫЙ КАБИНЕТ</h4>
            <p>Вы можете (<a class="blue_link_a" href="javascript:void(0)" onclick="openAuthorizePopup()">войти</a>) под своим логином и заказ привяжется к вашему аккаунту.</p>
            <p>Если у вас нет аккаунта, он будет создан автоматически. </p>
        <?else:?>
            <h4><img class="r_ico" src="<?=$this->GetFolder()?>/images/r_ico_1.png">&nbsp;&nbsp;&nbsp;ЛИЧНЫЙ КАБИНЕТ</h4>
            <p>Вы вошли как <a class="blue_link_a" href="/personal/main/"><?=$USER->GetFullName()?></a></p>
        <?endif;?>
    </div>
</div>
