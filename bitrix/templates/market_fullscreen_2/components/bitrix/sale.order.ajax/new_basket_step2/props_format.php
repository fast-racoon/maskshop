	...
	function PrintPropsForm($arSource = array(), $locationTemplate = ".default")
	{
		if (!empty($arSource))
		{
			...
			<div>
				...
				{
                                        if($arProperties['ID'] == 22)
                                        {
                                            $licensePropInfo = $arProperties;
                                            continue;
                                        }
					?>
				<div data-property-id-row="<?=intval(intval($arProperties["ID"]))?>" <?if(intval($arProperties["ID"]) == 25 || intval($arProperties["ID"]) == 26){?>style="display: none;"<?}?>>
						...
						{
							?>
							<div class="bx_block r3x1" id="<?=$arProperties["FIELD_NAME"]?>_block"
							    <?if($arProperties["REQUIRED"] == "Y"):?> data-text="Поле обязательно для заполнения."
								    <?if($arProperties["IS_EMAIL"] == "Y"):?>
                                        data-email="E-Mail указан некорректно."
                                        data-exist="Такой пользователь уже зарегистрирован. Войдите в свой аккаунт."
                                    <?endif?>
								<?endif?>>
								<input data-code="<?=$arProperties["CODE"]?>" class="<?if($arProperties["REQUIRED"] == 'Y'):?>field-req<?endif;if($arProperties["FIELD_NAME"] == "ORDER_PROP_3"):?> phones<?endif;?>" type="text" <?if($arProperties["FIELD_NAME"] != "ORDER_PROP_3"){?>maxlength="250" size="<?=$arProperties["SIZE1"]?>"<?}?> value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" <?if($arProperties["FIELD_NAME"] == "ORDER_PROP_3"):?>placeholder="+7"<?endif;?> />
								<?if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
									<div class="bx_description"><?=$arProperties["DESCRIPTION"]?></div>
								<?endif?>
						...
						elseif ($arProperties["TYPE"] == "TEXTAREA")
						{
							...
							?>
							<div class="bx_block r3x1" <?if($arProperties["REQUIRED"] == "Y"):?> data-text="Поле обязательно для заполнения."<?endif?>>
								<textarea class="<?=($arProperties["REQUIRED"] == 'Y' ? 'field-req' : false)?>" rows="<?=$rows?>" cols="<?=$arProperties["SIZE1"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>"><?=$arProperties["VALUE"]?></textarea>
								<?if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
									<div class="bx_description"><?=$arProperties["DESCRIPTION"]?></div>
								<?endif?>
                                <span class="field-error"></span>
...
*/
    ...
    margin-bottom: 15px;
    display: block;
    width: 100%;;">* - обязательно для заполнения</span>
                                        
                                <?global $BXRGeneral;?>
								
                                <div class="personal-data-accept">
									<input type="checkbox" name="ORDER_PROP_22" id="ORDER_PROP_22" checked <?//if ($licensePropInfo["VALUE"]=="on") echo " checked";?>>
                                    <span class="bx_sof_req">*</span>
                                    <label for="license" style="display:initial;">
                                        <?=$BXRGeneral['PAGES']['all']['alexkova.market:form.iblock']['PERSONAL_DATA_TEXT']?>
                                        <a target="_blank" href="<?=$BXRGeneral['PAGES']['all']['alexkova.market:form.iblock']['PERSONAL_DATA_URL']?>" style=" color: #7780fe; ">
