...
foreach ($arResult["MESSAGES"] as $res):
	...
	?><table cellspacing="0" border="0" class="reviews-post-table <?=($iCount == 1 ? "reviews-post-first " : "")?><?
		...
		{
			?>
			<div class="review-avatar" style="background-color: #ffffff;border: 1px solid #ffffff;"><?
				if(is_array($res["AVATAR"]) && array_key_exists("HTML", $res["AVATAR"])): ?><?=$res["AVATAR"]["HTML"]?><?
				else: ?><img src="<?=$this->GetFolder();?>/images/noavatar.gif" border="0" /><? endif;
			?></div>
		<?
		}
		?>
...
<div>
		...
		?></b>
		<span class='message-post-date' style="display: block; margin-left: 38px;"><?=$res["POST_DATE"]?></span>
</div>
	...
	<tbody>
	<tr><td>
		<div class="reviews-text" style="margin-left: 38px;" id="message_text_<?=$res["ID"]?>"><?=$res["POST_MESSAGE_TEXT"]?></div>
<?
...
<?
	...
	{
		?>
		<div class="reviews-post-reply-buttons" style="margin-left: 38px;"><noindex>
