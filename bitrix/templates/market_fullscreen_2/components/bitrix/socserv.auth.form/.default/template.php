	...
	<form method="post" name="bx_auth_services<?=$arParams["SUFFIX"]?>" target="_top" action="<?=$arParams["AUTH_URL"]?>">
		...
		<?if($arParams["~FOR_SPLIT"] != 'Y'):?>
			<div class="bx-auth-services">
				<?foreach($arAuthServices as $service):
//print_r($service);?>
					<div><a href="javascript:void(0)" onclick="<?echo $service["ONCLICK"]?>" id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>"><i class="bx-ss-icon <?=htmlspecialcharsbx($service["ICON"])?>"></i><b><?=htmlspecialcharsbx($service["NAME"])?></b></a></div>
				<?endforeach?>
			</div>
		<?endif;?>
