	...
	<form method="post" name="bx_auth_services<?=$arParams["SUFFIX"]?>" target="_top" action="<?=$arParams["AUTH_URL"]?>">
		...
		<?if($arParams["~FOR_SPLIT"] != 'Y'):?>
			<div class="bx-auth-services">
				<?foreach($arAuthServices as $service):
//print_r($service);?>
				<div style="display: inline-block; width: 85px; text-align: center;"><a href="javascript:void(0)" onclick="<?echo $service["ONCLICK"]?>" id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>"><img src="<?echo $this->GetFolder()."/imgs/".htmlspecialcharsbx($service["ICON"]).".png";?>" style="max-width: <?if($service["ICON"] == "odnoklassniki"){?>52px<?}else{?>50px<?}?>;"></a></div>
				<?endforeach?>
			</div>
		<?endif;?>
